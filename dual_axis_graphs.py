...
def time_miles_dual(data):
    ...
    #Pace on right axis
    ax2 = ax1.twinx()
    ax2.plot(weekly["Week"], weekly["Time"], color="royalblue", label="Total Time Running (mins)")
    ax2.set_ylabel("Total Time per Week", color="royalblue")
    ax2.tick_params(axis="y", labelcolor="royalblue")

    plt.title("Weekly Mileage and Time Trend")
